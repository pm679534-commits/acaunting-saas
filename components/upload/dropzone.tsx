...
export function Dropzone({ limitExceeded, currentUsage, monthlyLimit, planName }: DropzoneProps) {
  ...
  const startUpload = useCallback(async () => {
    ...
    try {
      ...
      if (!uploadRes.ok) {
        ...
        // Check for LIMIT_EXCEEDED error code from API
        ...
        throw new Error(body.message || body.error || "Yükləmə xətası")
      }
  ...
  return (
    <>
      <div className="max-w-2xl">
        ...
        {!file ? (
          <div
            onDragOver={(e) => {
              e.preventDefault()
              if (!limitExceeded) setDragOver(true)
            }}
            onDragLeave={() => setDragOver(false)}
            onDrop={handleDrop}
            onClick={handleClick}
            className={cn(
              "relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed p-16 transition-all duration-200 bg-white",
              limitExceeded
                ? "border-slate-200 bg-slate-50 cursor-not-allowed opacity-60"
                : dragOver
                ? "border-primary bg-primary/5 scale-[1.01] cursor-pointer"
                : "border-slate-300 hover:border-primary/60 hover:bg-slate-50/50 cursor-pointer"
            )}
          >
            <input
              ref={inputRef}
            ...
            <p className="text-base font-medium text-slate-900 mb-1">
              {limitExceeded ? "Yükləmə mümkün deyil" : "Faylı bura sürükleyin və ya seçin"}
            </p>
            <p className="text-sm text-slate-500">
              {limitExceeded ? `Limit: ${currentUsage}/${monthlyLimit}` : "JPG, PNG, WebP, PDF · Maks. 50 MB"}
            </p>
            {error && !limitExceeded && (
      ...
      <Dialog open={showLimitDialog} onOpenChange={setShowLimitDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Aylıq limit dolmuşdur</DialogTitle>
            <DialogDescription>
              Bu ay {currentUsage}/{monthlyLimit} sənəd istifadə etmisiniz ({planName} planı).
              Daha çox sənəd yükləmək üçün planınızı yüksəldin.
            </DialogDescription>
          </DialogHeader>
